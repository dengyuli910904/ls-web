...
@section('content')
<div class="pd-20">
	<form action="{{ route('menu.store') }}" method="post" class="form form-horizontal" id="form-menu-create">
		@if (count($errors) > 0)
			<div class="row cl">
		...
		@endif
			...
			<div class="row cl">
				...
				<div class="formControls col-10">
					<input type="text" class="input-text" value="{{ old('url', '') }}" placeholder="链接" name="url" >
				</div>
			</div>
			<div class="row cl">
							...
							<select name="permission_id" class="select" required="required" id="permission_list">
								...
								@foreach($data['permissions'] as $permission)
									<option value="{{ $permission['id'] }}"
											@if(old('permission_id', 0) == $permission['id'])
											selected="selected"
											@endif
