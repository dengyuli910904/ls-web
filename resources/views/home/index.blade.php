@extends('home.layouts.web')

@section('content')
<!-- Search -->
    <!-- <div class="t-c header-title pd-b-10">
       <h2>
           海南体育赛事官网
       </h2>
    </div> -->
    <!-- End Search -->

    <!-- Header -->
    <!-- <header>
        <div class="container">
         <div class="intro-text">
                <div class="intro-lead-in">&nbsp;</div>
                <div class="intro-heading">&nbsp;</div>
            </div>
        </div>
    </header> -->
    
    
      
    <section id="index-newlist" class="pd-t-50 pd-b-20">
        <div class="container">
            <div class="row">
                <div class="col-md-12 pd-b-20">
                    <h4><a href="#"><span>新闻动态</span> | <span>NEWS </span></a></h4>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰
                </div>
                <div class="col-md-6">
                    科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰
                </div>
                <div class="col-md-6">
                    科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰
                </div>
                <div class="col-md-6">
                    科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰
                </div>
                <div class="col-md-6">
                    科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰
                </div>
                <div class="col-md-6">
                    科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰
                </div>
                <div class="col-md-6">
                    科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰
                </div>
                <div class="col-md-6">
                    科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰对兰特一句话激怒球迷杜兰
                </div>
                <div class="col-md-6">
                    科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰
                </div>
            </div>
            <div class="row">
                <div class="col-md-6 t-r">&nbsp;&nbsp;</div>
                <div class="col-md-6 t-r">MORE</div>
            </div>
        </div>
    </section>
        <!-- 专题 -->
    <section id="index-specialchannel" class="pd-t-50 pd-b-20">
        <div class="container">
            <div class="row">
                <div class="col-md-12 pd-b-20">
                    <h4><a href="#"><span>专题频道</span> | <span>SPECIAL CHANNEL </span></a></h4>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <img src="{{ asset('images/img.png')}}" class="img-responsive" width="70%">
                </div>
                <div class="col-md-6">
                    <div class="col-md-12">
                        科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰
                    </div>
                    <div class="col-md-12">
                        科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰
                    </div>
                    <div class="col-md-12">
                        科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰
                    </div>
                    <div class="col-md-12">
                        科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰
                    </div>
                    <div class="col-md-12">
                        科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰
                    </div>
                    <div class="col-md-12">
                        科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰
                    </div>
                    <div class="col-md-12">
                        科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰
                    </div>
                    <div class="col-md-12">
                        科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰
                    </div>
                    <div class="col-md-12">
                        科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰
                    </div>
                    <div class="col-md-12">
                        科尔：詹姆士是绝对领袖卢太水t对兰特一句话激怒球迷杜兰
                    </div>
                </div>
                    
            </div>
            
        </div>
    </section>
    <section id="index-newstype" class="pd-t-40 pd-b-50">
        <div class="container">
            <div class="col-md-4">
                <span class="fa-stack fa-4x">
                    <i class="fa fa-circle fa-stack-2x text-primary"></i>
                    <i class="fa fa-shopping-cart fa-stack-1x fa-inverse"></i>
                </span>
                <h4 class="service-heading">
                    <div class="t-c">
                       <div class="t-c circle-icon on">
                           <div class="circle-icon-inner">
                               1
                           </div>
                       </div>
                    </div>
                    <p class="t-c icon-title">体育赛事</p>
                </h4>
                <p class="text-muted">企业能够获取和利用消费者数据，并利用只针对个性化数据设计的复杂的算法，为客户提供个性化的客户体验.</p>
            </div>
            <div class="col-md-4">
                <span class="fa-stack fa-4x">
                    <i class="fa fa-circle fa-stack-2x text-primary"></i>
                    <i class="fa fa-shopping-cart fa-stack-1x fa-inverse"></i>
                </span>
                <h4 class="service-heading">
                    <p class="t-c">
                       <div class="t-c circle-icon">
                           <div class="circle-icon-inner">
                               2
                           </div>
                       </div>
                    </p>
                    <p class="t-c icon-title">赛事新闻</p>
                </h4>
                <p class="text-muted">企业能够获取和利用消费者数据，并利用只针对个性化数据设计的复杂的算法，为客户提供个性化的客户体验.</p>
            </div>


            <div class="col-md-4">
                <span class="fa-stack fa-4x">
                    <i class="fa fa-circle fa-stack-2x text-primary"></i>
                    <i class="fa fa-shopping-cart fa-stack-1x fa-inverse"></i>
                </span>
                <h4 class="service-heading">
                    <p class="t-c">
                       <div class="t-c circle-icon">
                           <div class="circle-icon-inner">
                               3
                           </div>
                       </div>
                    </p>
                    <p class="t-c icon-title">体育百科</p>
                </h4>
                <p class="text-muted">企业能够获取和利用消费者数据，并利用只针对个性化数据设计的复杂的算法，为客户提供个性化的客户体验.</p>
            </div>
        </div>
    </section>
    <section id="eventnews" class="pd-t-20 pd-b-50">
        <div class="container">
            <div class="row">
                <div class="col-md-12 pd-b-20">
                    <h4><a href="#"><span>新闻赛事</span> | <span>EVENT NEWS</span></a></h4>
                </div>
            </div>
            <div class="row">

                <div class="col-md-12 col-xs-12 item">
                    <div class="col-md-8 col-xs-12">
                        <div class="date-item col-md-2 col-xs-3">
                                <p>06/15</p>
                                <p>MARCH</p>
                        </div>
                        <div class="news-item col-md-10 col-xs-9">
                            <h5>网球比赛</h5>
                            <p>网球比赛网球比赛网球比赛网球比赛网球比赛网球比赛网球比赛网球比赛</p>
                        </div>
                    </div>
                    <div class="col-md-4 hidden-xs">
                        <img src="{{ asset('images/img_1.png')}}" class="img-responsive" width="100%">
                    </div>
                </div>
                <div class="col-md-12 col-xs-12 item">
                    <div class="col-md-8 col-xs-12">
                        <div class="date-item col-md-2 col-xs-3">
                                <p>06/15</p>
                                <p>MARCH</p>
                        </div>
                        <div class="news-item col-md-10 col-xs-9">
                            <h5>网球比赛</h5>
                            <p>网球比赛网球比赛网球比赛网球比赛网球比赛网球比赛网球比赛网球比赛</p>
                        </div>
                    </div>
                    <div class="col-md-4 hidden-xs">
                        <img src="{{ asset('images/img_2.png')}}" class="img-responsive" width="100%">
                    </div>
                </div>
                <div class="col-md-12 col-xs-12 item">
                    <div class="col-md-8 col-xs-12">
                        <div class="date-item col-md-2 col-xs-3">
                                <p>06/15</p>
                                <p>MARCH</p>
                        </div>
                        <div class="news-item col-md-10 col-xs-9">
                            <h5>网球比赛</h5>
                            <p>网球比赛网球比赛网球比赛网球比赛网球比赛网球比赛网球比赛网球比赛</p>
                        </div>
                    </div>
                    <div class="col-md-4 hidden-xs">
                        <img src="{{ asset('images/img_3.png')}}" class="img-responsive" width="100%">
                    </div>
                </div>

            </div>
        </div>
    </section>
    <script type="text/javascript">
        $('.carousel').carousel();
    </script>
@endsection
